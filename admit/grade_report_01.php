<!DOCTYPE html>
 <head>
	...
	<?php
		...
		$target_url = "student_page_grade.php?session={$session}&year={$year}&student_id=";
		$results = get_grade_report($benchmark, $year, $session, $target_url);
		$heading = "<h3>Student Grades for Session {$session}, {$year}</h3>";
		$table = $heading."<br>".$results;
		$css_filepath = '../css/pdf_style.css';
		$filename = "Student-grades-Session{$session}-{$year}.pdf";
